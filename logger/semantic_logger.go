...
var logLevelToString map[LogLevel]string = map[LogLevel]string{TRACE: "trace", DEBUG: "debug", INFO: "info", WARN: "warn", ERROR: "error", FATAL: "fatal"}
var stringToLogLevel map[string]LogLevel = map[string]LogLevel{"trace": TRACE, "debug": DEBUG, "info": INFO, "warn": WARN, "error": ERROR, "fatal": FATAL}

func (ll LogLevel) String() string {
	v, ok := logLevelToString[ll]
	if !ok {
		return ""
	}
	return v
...
func LogLevelByName(s string) LogLevel {
	v, ok := stringToLogLevel[s]
	if !ok {
		return UNDEFINED
	}
	return v
